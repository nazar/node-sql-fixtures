...
function buildInsertPromise(knex, tableName, records, unique, showWarning) {
  ...
  return getInsertableRecords(knex, tableName, insertRecords, unique).then(function(insertableRecords) {
    ...
    if (isPostgres(knex)) {
      const insertChunks = _.chunk(insertRecords, 1000);

      return bluebird.mapSeries(insertChunks, (chunk) => {
        return knex(tableName).returning('*').insert(chunk);
      }).then((insertResults) => {
        const flatResults = _.flatten(insertResults);

        return assembleFinalResult(flatResults);
      })
    } else {
